...
Để đảm bảo tính mô-đun hóa và tránh lặp lại hoặc phân rã mã nguồn không rõ ràng (như tình trạng của `PaginationBar` hay `DocumentCard`), toàn bộ các components trong dự án phải tuân theo quy tắc phân chia địa lý sau:
...
- **Ví dụ**: `DocumentCard` hay `DepartmentMajorSubjectPicker` chỉ thuộc về nghiệp vụ tài nguyên và bộ chọn phân lớp ngành học.
